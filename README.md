...
[check this](https://docs.ethers.io/v5/api/providers)

Now that we've made our wallet connection, we're ready to do some blockchain interactions with our DApp. Let's take a look at what we can do and how we can do it. So first things first, in order to interact with the blockchain in terms of how we want to do it, let's go ahead and start our local node. That'll allow us to get some ETH and do things that we wouldn't do unless we had a ton of ether on the main net. So I'm going to go to the terminal and I'm going to create a new window. I've got my DApp running in one window. And in my second window, I'm going to go into the back end of my DApp. Now, I'm going to go ahead and start this. And there's a whole video that talks about how to deploy smart contracts and whatnot in an earlier section. So what we can do is we can say, npx hardhat node. And this will actually fire up a local node of Ethereum through Hardhat. And we can actually talk to it. And you'll see, once it connects and establishes it, it's going to show us a bunch of accounts. And look, at each of these accounts, have 10,000 ETH already assigned. So I'm going to copy private key. And then I can go back to my browser and I can go to my wallet. Now, the main thing is I need to be able to make sure that I can connect to the local test net. And here I'm connected to Ropsten. And notice that it's showing all of these test nets. Now, you can turn on and off the ability to see these. If you don't see them, you should see an option to hide, show test nets. Make sure you turn that to show. And I'm going to go ahead and go to Add Network. Now I want to add our local network. So I go to Networks, and I'll create Add a network and we'll call it localhost. It's localhost:8545. And the chain ID is going to be 31337. And then the currency symbol is going to be ETH. Okay, so there you have it. HTTP, localhost, 8545, and the currency symbol's ETH. Now, we'll go ahead and save it. All right, so now, it's got my localhost. You can see, and when I go in here, I should also be connected. Yeah, I'm connected to the local host. Now, check this out. I can go and say, up here... So you can either do it in this mega browser one. This is like a big version of this little guy. It's basically a big version, but I'm going to go ahead and import this account. So if you're here, Import Account, and then select type, Private Key. And then I paste my private key, and I say Import. Now, look I imported it and look... Woo-hoo. I'm rich. So now, I've got my 10,000 ETH that I could start playing around with this at my localhost. All right, so once I'm connected, let's take a look at this ethers library here. All right, so when you look at the Providers, you can see I can get balanced from an address. There's a bunch of things I can do. I can go and get blocks, and I can get transaction counts from a specific address so I can see what's happened. There are a lot of things that I can get from the blockchain. Now, if we want to get more information about wallet, we're going to have to go into the Signer, right? And that's what we used last time, is we used the Signer to connect, right? So one of the things you can do is I can go and see if there's enough ETH in someone's wallet before they mint, right? Now, in this case, our particular minting object, it's free. So we don't have to worry about that. But if you are minting something for, say, 0.2 ETH or 1.6 ETH or whatever it is, you might want to check and make sure that they have that balance before you call the mint function because it won't work if they don't have enough money. All right. So let's go ahead and go into our code. So here's our code. And then what I'm going to do is just in my wallet connection, I'm going to go ahead and take a look at the balance. All right, so now I'm going to create a little variable called balance, and I'm going to say, well, let's let that equal to signer.getBalance, right? So that then is going to get the balance from the address that's connected. You see enough? Now I'm going to go ahead and type in console.log. All right, so now we've saved it. Now, let's go ahead and go to our DApp. Oh, and it loads up. So first thing, I'll connect my wallet. Now let's see what I've got here. Okay, so notice the wallet that came up didn't match the wallet that I wanted, right? I want this particular wallet. And if I look at my line, it's the one that ends in 2266. So I want to make sure that's the one that's connected, and note that it's not. This other one is connected. So I'm going to use my wallet here to change so that that one's connected, okay? So now this one should be connected to my DApp. So let's go ahead and refresh it, and see what happened. 2266, so it did work, okay. So now we're connected. Actually, what I want to do is I want to view JavaScript Console, and let's go ahead and take a look here. So look, when it connected to 2266, it has this big number. And look at the value, it's in hex, okay? So it's a big number in hex. So when you transfer amount of ether inside of Ethereum, it is going to be in wei, which is the smallest, absolute, smallest unit you can possibly use. All right, so the smallest unit is wei, which is 18 decimal places to the right of one ether. So 10,000 ether is quite a lot in terms of how many wei, so we need to convert it. And it's also in base 16, which is not really how the human brain works. So we can actually use some utilities to translate this. And if you look here, we have this lovely one called formatEther, which will change it to ETH. So it'll be a decimal point to tell me 0.00, whatever ETH. Now in this case, it'll be 10,000. So I'm going to go back into my code. And instead of logging the balance, I'm going to await, and I'm using this because it's asynchronous call, because I actually need to go out and I need to format it using the ethers library. So ethers.utils and then formatEther and then make sure I get this over there, in here. All right. So that should do it. So let's go ahead and go back and refresh our React App. Now we'll connect our wallet. It's connected. And you can see it outputs 10,000. And that's what it read from the wallet which it actually is getting from the Ethereum blockchain. So you can see once you get that ethers connection, we can get all kinds of information from the blockchain. So in this video, we got set up with Hardhat testing node, and we learned to read our wallet contents. That's pretty cool. Next, let's have a look at what some of these other providers are all about. 

[about wei](docs.ethers.io/v5/api/utils/display-logic)

![image](https://user-images.githubusercontent.com/53083156/225755394-4135b8eb-8b99-4dee-86a8-13394e910edd.png)
![image](https://user-images.githubusercontent.com/53083156/225755641-0fc5282d-41d5-43fb-a159-fafdefb251e1.png)

###  Using providers 

One of the reasons I liked ethers.js is because of its support of different APIs through their provider. Let's look at some of these providers and when and why you might want to use them. If you look at the provider's section of the ethers doc, you might have noticed there's a list of some API providers that you can talk to. Now, the beauty of ethers is that no matter which one you use, they basically all use the same API, which means it's pretty easy to switch from one another based on your needs. In fact, you can set up a default one and then have other fallback providers so that you have some redundancy, which is again, the power of decentralization. You want to have that redundancy. So these providers, I would say, are the equivalent of using something like AWS or Google Cloud for hosting, but it's Ethereum. They give you the reliability and speed of a managed redundant network of Ethereum nodes. It's a great practice for making smooth-running dapps. And many of the dapps you use take advantage of them to enhance user experience. Things like OpenSea, and SuperRare, and (indistinct) They all use them because they work. So ethers has a predefined set of API keys that will work and are fine for testing. But if you go into production, you're going to want to make sure you go and get your own. So [Alchemy](https://www.alchemy.com) is a very popular one that you see. And you can see there's a number of good projects that use these. They have a lot of tools in addition to ethers.js that can support you in building them; dashboards and things so you can easily change things. And there's new things coming out all the time; push notifications and whatnot. [Infura](https://infura.io) is another very popular one that's used by a lot of different projects. Highly available. They also have IPFS in Infura, which is a nice add if I do say so. And some of these actually support some layer 2 networks, like Polygon and Arbitrum, which is nice because those are sort of built around Ethereum to save on gas prices. Etherscan, which is the popular block explorer, they have their own API that you can take advantage of if you want to. And then there are new ones coming up all the time, like Pokt. They have their own POKT token that you can use. And they actually support some other chains in addition to Ethereum as well. And Ankr is another one that's up and coming that you'll see. So there's lots of great reasons to use these. And each of them has their own different sort of specialty, depending on the type of dapp you're building, and what your constraints are, and if you're supporting multi-chain, if you're using layer 2, those kind of things. So very important to look into these in terms of what you want to use. And all these, by the way, that I've mentioned, they're all supported out of the box by ethers.js. So you just need to get a key, and plug it in. And boom, you can use that. And then, of course, there are many other tools that will be available within these environments, depending which one you're using. And you may or may not need different JavaScript libraries and different things to activate those. Now, another great way to access data on the Ethereum blockchain is by using The [Graph](https://thegraph.com/en). If you're familiar with GraphQL, The Graph is basically an indexing protocol that uses GraphQL so that you can call them into your dapps. So it's its own project. It has its own token. It's a decentralized project. And you can create queries. And they have their own console in here. You can create your own subgraphs that you can then query when you need to. And again, this is going to speed up your experience. It's going to make it a lot faster to read from the blockchain and get data that you need. In addition, there are ways that you can make crypto off of this by hosting nodes, by becoming a curator, and even by creating your own subgraphs. So very interesting project. It's a whole topic in and of itself, but it's very popular. And I definitely recommend looking into this because it's a good way to get data for the blockchain. Overall, using these third-party providers is a great way to enhance the performance of your dapps. The nature of decentralization is that it's very slow, but these kind of tools can be a great intermediator. So that way you can give your users a better and faster experience, but it's still going out to the blockchain. Ultimately, all of these things sync to the blockchain and they're getting blockchain data. So overall, worthwhile looking into. And The Graph, of course, is a great way to get blockchain data in GraphQL format. 

###  Calling a smart contract function 

We're finally at the moment we've all been waiting for, minting our NFT. So, to do so, we need to make sure that we've got our front end running and that we've got our Ethereum node running and essentially we're ready to go and we've deployed our contract. Well, I haven't done that yet. So, I'm going to go ahead and open a new window. And this is going to the root of my dapp, which is where my smart contracts are. And we went over this in a video earlier, but we're just going to do it again just to make sure we've all got it right. And then, I'm going to type an NPX hard hat, run network, local host, to make sure it deploys to the local host. And we're going to run scripts/mint-script.JS, which is the one that deploys our minting contract. So, here we go. Churn, churn, churn, chugga chugga chugga churn. All right. Compiled. And it's deployed to that address. So, beautiful thing is I've got that address. So, that's where I'm going to copy that. That's where we're going to go to. Now, let's go into our code and let's look at a couple things real quick. So, right now I'm in the Solidity project and you'll notice there's this Artifacts folder. Now in here, there's my contracts. And there's actually a folder called "SOL." There's this item here called "BinaryvilleRobotsNFT.JSON." And now, have a look here. And one of the things you can see is this ABI. Now, I'm going to need this. I'm actually going to cut this from the beginning of that square brace down to the closing square brace right here. So, I'm going to cut that out, not right now, but later I'm going to, and you'll know what I'm talking about. So, take a note of that. Okay. So, now here I am back in my nav bar. Now, I have my mint function and this is what I'm going to fill in now. Okay. I'm going to need this contract address. So, there it is. So, now I'm going to take that contract address and put in here. Oh, look, it's the same. That's interesting. So, you can see I've added my contract address in here. And what I'm doing is I'm creating a new constant that's representing my contract, using the ethers.contract object, and then I'm passing this iface, and the signer, right? Now, the iface. What is that, you ask? Well, that is what I'm getting from the ABI. So, I'm creating a constant, jsonAbi, which is this ABI. And the ABI is really just a representation of the contract. Now, ideally you want to put it in human readable format where it just basically says what the different functions are. So, human readable ABI looks something like this. It has the constructor, it has the variables, but it looks like functions that are written. JSON isn't as human readable, it's defining the same thing but it's really meant to be parsed by a parser. So, what I'm going to do is I'm actually going to take it and I'm going to translate it using this interface here. And then, this will change it to essentially human readable format behind the scenes, and we cannot put that, but for now, you don't need to worry about that. Just know that I'm basically passing the ABI there. So, the ABI, that and the signer, will essentially get the contract set up. And then, I'm going to go ahead and create NFT data. And then, I'm going to call the mint NFT function. Now, instead of this, I'm going to pass URL. The IPFS:// will work if you're using a resolver that will resolve that. Now, a lot of browsers, AKA user agents, don't do that. So, I'm going to go ahead and get my address from pinata. So, this is it here. And I can just copy that, 'cause this is exactly that. And I can put that in here as the token URI, and that's the metadata, keep in mind, which will refer to the image that we have. All right. So, everything's all set up. Now, I can go to my DApp. Oh, something's not working. So, let's go and have a look. Okay. So, we have a problem. Problem, an expected reserve word, "await." Ah, I see what the problem is. Our function should be an async function, 'cause we're using await. All right. Let's have a look again. Ah, interface and format types. Yes, yes, yes. You need to make sure to import those. All right. Now, let's see. All right. So, now it compiled. So, let's reload it and there's our beautiful DApp. So, connect our wallet. Boom, it's connected. We'll go to mint. Now, if I'm lucky, it should bring up the wallet. It did, and it's telling me some information about gas and all that. Yes, I can close that and how much it's going to cost and I'm going to confirm it and it's off and running. Now, we can view JavaScript console. And it up with this, which tells me that it succeeded. Definitely took some gas, mint NFT. By the zero E was free and it happened, tells me information about it. It's been confirmed. So, that's it. We've now minted our NFT successfully.
