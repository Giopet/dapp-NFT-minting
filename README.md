...
So let's take a look at the nodes that are running. So this is an interesting website because it can tell us everything that's currently running. It tells us all about the Ethereum network. Now, as I said, it all kind of functions as one and it syncs with one another. When you make a transaction, it will go out and make a transaction to some particular node of Ethereum. But then that will sync to other nodes. So here you can see what clients are being run and I'll talk about clients in a second. You can see what the countries are it's being run in, what the sync status is. How much of it is synced and how much of it is currently syncing in terms of the block, and more information. And there's another interesting website called ethviewer.live. And this gives us a real-time view into what's happening on the blockchain. So each of these are different miners that are processing data and it sort of shows you what's happening in terms of mining these blocks. And it's fairly cool, somewhat transfixing to watch just to get a feel of what's going on in real time. So there are two basic types of clients that you can run. You can run what's called the execution client, which is the current consensus methodology, which is proof-of-work. That is the old-fashioned like Bitcoin way of mining using processing power. And then there's the consensus clients, which is also called the beacon chain. It used to be called Ethereum 2.0. And this uses proof-of-stake. Now, proof-of-stake operates a little differently. You can essentially lock up some of your Ether and that is essentially what will create new Ether in the mining process is these. Everyone's who's locked up their Ether will get rewards. So as at the writing of this course, this merge hasn't happened. Eventually, there will just be one and that will be the beacon chain. However, that hasn't happened yet. It's been in the works for about five years but it pretty much should happen by the end of the year. So we'll see. So one last note about clients. There are a number of different clients out there and I won't get into them. Typically, most people, as you saw on that website use Geth, which is written in Go. But there are others for people who have preferences otherwise. The other piece to know about is you can run clients that are specifically for running testing. And they'll run in the Ethereum client locally. Ganache and Hardhat are testing environments that can be used for just this purpose. And that's what we're going to use when we get into running our tests. Additionally, there are other networks available in addition to the Mainnet that can be used specifically for testing. So they're kind of like somewhere between your local testing environment and the final Mainnet, and it's, of course, a good idea to test your smart contracts extremely well. So Ethereum is a blockchain for programming. And its core functioning is possible because of the EVM. And that's what processes are compiled smart contract. It's available in a number of different version but Geth is pretty much by far and away the most popular. Because Ethereum can process what we call Turing complete programming, which means it can do loops and iterate and things, it gives Ethereum a lot of power and make it a unique and really good platform for decentralized application development. 
